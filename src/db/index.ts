...
export function getMergeHistory(repo: string, limit: number = 100): MergeHistoryEntry[] {
  const db = getDatabase();
  const rows = db
    .query(
...
export function insertCIJob(job: {
  ...
  const result = db.run(
    `INSERT INTO ci_jobs (repo, branch, head_commit, status, log_path, started_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [job.repo, job.branch, job.headCommit, job.status, job.logPath, job.startedAt.toISOString()]
  );
...
export function getLatestCIJob(repo: string, branch: string, headCommit: string): CIJob | null {
  const db = getDatabase();
  const row = db
    .query(
      `SELECT * FROM ci_jobs
...
export function cancelPendingJobs(repo: string, branch: string, exceptJobId?: number): void {
  const db = getDatabase();
