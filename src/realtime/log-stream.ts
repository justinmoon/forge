...
const subscribers = new Map<number, Set<ReadableStreamDefaultController<Uint8Array>>>();
...
function addSubscriber(jobId: number, controller: ReadableStreamDefaultController<Uint8Array>): void {
...
function removeSubscriber(jobId: number, controller: ReadableStreamDefaultController<Uint8Array> | null): void {
...
export function appendJobLogChunk(jobId: number, chunk: string | Uint8Array): void {
...
export function streamJobLog(jobId: number): Response {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      controller.enqueue(encodeEvent('log', JSON.stringify({ html: initialHtml })));
    },
    cancel() {
